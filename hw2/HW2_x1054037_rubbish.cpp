...
public:
	...
	void scheduler (Task task, int n)
	{
		...
		for(; rest; first = false)
		{
			...
			// 	memcpy(buffer0.data(), &tasks[i++], sizeof(Task))
			// 	finish0 = false;
			// 	mutex.unlock();
			// 	continue;
			// }
			...
			// Recv a result
			MPI_Recv(buffer, count, MPI_BYTE, src, 0, MPI_COMM_WORLD, &status);
	...
	MPI_Request request;
	MPI_Status status;
	void beginGetNextTask()
	{
		MPI_Irecv(&task, sizeof(Task), MPI_BYTE, MASTER_RANK, 0, MPI_COMM_WORLD, &request);
	}
	Task endGetNextTask()
	{
...
void dynamicMPI (Task task_all, int thread_num, const char* output)
{
	...
	#pragma omp parallel
	{
		...
		#pragma omp single 
		{
			...
			// {
			// 	Task task = ts.endGetNextTask();
			// 	if(task.rest == 0)
			// 	{
			// 		ts.endSendTaskResult();
			// 		break;
			// 	}
			// 	ts.beginGetNextTask();
			...
			// 	if(!first) ts.endSendTaskResult();
			// 	ts.beginSendTaskResult(task, data.data());
			// }
...
void staticMPI (Task task_all, int thread_num, const char* output)
{
	...
	int n_proc = n / size + 1;
	auto tasks_proc = task_all.splitN_Seq(size);
